...
public sealed class ChatService(IChatRepository repository, IClock clock)
{
    ...
    public Task<ChatMessageDto?> AddMessageAsync(Guid publicId, string userEmail, AddChatMessageRequest request, CancellationToken cancellationToken)
    {
        ...
        return repository.AddMessageAsync(publicId, NormalizeRequired(userEmail, "User email is required.", 200), request with { Role = role, Content = content }, clock.UtcNow, cancellationToken);
    }
    ...
    {
        _ = NormalizeRequired(request.ToolName, "Tool name is required.", 100);
        _ = NormalizeRequired(request.AllowedSurface, "Allowed surface is required.", 100);
        _ = NormalizeRequired(request.ArgumentsJson, "Tool arguments are required.", 20000);
        return repository.AddToolCallAsync(publicId, NormalizeRequired(userEmail, "User email is required.", 200), request, clock.UtcNow, cancellationToken);
    }
    ...
    {
        _ = NormalizeRequired(request.ContextType, "Context type is required.", 50);
        _ = NormalizeRequired(request.EntityType, "Entity type is required.", 50);
        return repository.AddContextItemAsync(publicId, NormalizeRequired(userEmail, "User email is required.", 200), request, clock.UtcNow, cancellationToken);
    }
