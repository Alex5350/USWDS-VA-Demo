...
Full local setup for the VA OIG FWA Risk Triage & Reporting Portal demo: prerequisites, SQL
Server in Docker, database bootstrap, backend, frontend, the AI chat assistant key, demo
authentication, and troubleshooting. The product overview lives in the
[README](../README.md); the engineering deep dive in [TECHNICAL.md](../TECHNICAL.md).

The fastest path for reviewers who only want to see the interface: `bun run dev` from
`src/client` alone renders the full portal from its embedded offline dataset. Everything below
stands up the live SQL-backed path.
